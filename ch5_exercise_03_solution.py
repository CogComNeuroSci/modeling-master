...
# create an array that will contain all the accuracy scores for all simulations
    ...
    # simulation
simulations_scores         = np.zeros((standard_deviations.shape[0], simulation_number))
...
for simulation in range(simulation_number): 
    ...
    for std in standard_deviations:
            ...
            # of this
            accuracy_scores[cycle_number] = accuracy_score(outputs, y_pred) * 100
    ...
    simulations_scores[simulation] = accuracy_scores
...
# show we what you got
    # we expect 50 arrays, each containing scores between 0 and 100
'''
print(simulations_scores)
'''
#%%
'''
...
# with each cycle
mean_accuracy = np.mean(simulations_scores, axis = 0)
median_accuracy = np.median(simulations_scores, axis = 0)
