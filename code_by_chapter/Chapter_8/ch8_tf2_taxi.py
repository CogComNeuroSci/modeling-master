    ...
    def build_network(self):
        def PG_loss(y_true, y_pred):
            ...
            for loop in range(self.max_n_step):
                sum_total +=  advantage[loop]*K.log(y_pred[loop, action_true[loop]])
            return -sum_total
#           return -K.log(y_pred[: , action_true] + 1e-5) * advantage

        model = tf.keras.Sequential([ 
                tf.keras.layers.Dense(self.n_actions, input_shape = (self.n_states,), activation = "linear", name = "layer")
		    	] )
        model.build()
    ...
    def sample(self, state):
        ...
        y = self.network.predict(state_array)
        prob = np.squeeze(softmax(y))
...
def learn_w(env, n_loop: int = 100, max_n_step: int = 200, input_dim: int = 4):
    ...
    while not stop_crit:
        ...
        n_step, done = 0, False
        state = env.reset()
        states  = np.zeros((max_n_step, 1))
        actions = np.zeros((max_n_step, 1)) # to construct y_buffer
        rewards = np.zeros((max_n_step, 1)) # to construct y_buffer
...
if __name__ == "__main__":
    ...
    rl_agent = PG_Agent(n_states = env.observation_space.n, n_actions = env.action_space.n, \
                           lr = 0.1, gamma = 0.9, max_n_step = 100)
    if load_model:
        rl_agent.network = tf.keras.models.load_model(os.getcwd()+"/model_taxi")
    if train_model:
        lc, solved = learn_w(env, n_loop = 20, input_dim = env.observation_space.n, max_n_step = rl_agent.max_n_step)
    if save_model:
