...
class PG_Agent_AC(PG_Agent):
    """inherits from the policy gradient(PG) class, but adds actor critic (AC)"""
...
#        self.y_buffer[:, 1] = (self.y_buffer[:, 1] - avg)/(std + int(std == 0))  	    
    ...
    def learn(self, n_step, verbose: bool = True):
       	self.network.train_on_batch(self.x_buffer, self.y_buffer) # y_buffer is generated by the critic
