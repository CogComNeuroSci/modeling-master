...
data       = np.vstack([np.hstack([np.ones((n_stim//2,n_dim//2)), np.zeros((n_stim//2,n_dim//2))])  , 
					   np.hstack([np.zeros((n_stim//2,n_dim//2)), np.ones((n_stim//2,n_dim//2))])]) # block data
...
for loop in indx[1:]:
    ...
    for class_loop in range(n_class):
        ...
        prior = np.sum(assignment[:,class_loop]) # this term is proportional to the prior (which is good enough)	
