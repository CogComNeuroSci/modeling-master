...
with tf.Session() as sess:
    ...
    for epoch in range(epochs):
        ...
        if not epoch%grain:
            w = sess.run(W)
            b = sess.run(B)
            c[int(epoch/grain)] = sess.run(cost, feed_dict = {X: train_x, Y: train_y})
            print("cost = {:.2f}".format(c[int(epoch/grain)]))

# error curve
plt.plot(range(int(epochs/grain)), c, color = "black")